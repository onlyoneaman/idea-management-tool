...
const CreateIdeaApi = async (description: string, bucket_id: number | null, color: string) => {
  let apiResponse;
  if (ApiConstants.RUN_PROD_SERVER || process.env.NODE_ENV !== "development") {
    apiResponse = await PostRequest(ServerURL("/idea/new"), {
      description, bucket_id, color
    })
  } else {
