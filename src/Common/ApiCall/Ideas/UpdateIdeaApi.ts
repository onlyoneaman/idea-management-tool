...
const UpdateIdeaApi = async (description: string, bucket_id: number | null | undefined, idea_id: number) => {
  let apiResponse;
  if (ApiConstants.RUN_PROD_SERVER || process.env.NODE_ENV !== "development") {
    apiResponse = await PostRequest(ServerURL("/idea/update"), {
      description, bucket_id, idea_id
    })
  } else {
